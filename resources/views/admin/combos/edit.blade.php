...
@section('title', 'Editar Combo / Promoción')
    ...
    <div class="col-lg-10">
        <div class="card shadow-sm border-0">
            <div class="card-body p-4">
                <form action="{{ route('admin.combos.update', $combo->id) }}" method="POST" enctype="multipart/form-data" id="combo-form">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="row g-3">
                        <div class="col-md-6">
                            ...
                            <input type="text" name="nombre" class="form-control" placeholder="Ej: Combo Almuerzo Familiar" value="{{ old('nombre', $combo->nombre) }}" required>
                        </div>
                        ...
                        <div class="col-md-6">
                            ...
                            @if($combo->imagen)
                                <div class="mt-2">
                                    ...
                                    <img src="{{ asset('storage/' . $combo->imagen) }}" alt="{{ $combo->nombre }}" class="img-thumbnail mt-1" style="height: 80px; object-fit: cover;">
                                </div>
                            @endif
                        </div>
                        ...
                        <div class="col-12">
                            ...
                            <textarea name="descripcion" class="form-control" rows="2" placeholder="Ej: Incluye 2 platos de la casa y 1 refresco gratis. Válido fines de semana.">{{ old('descripcion', $combo->descripcion) }}</textarea>
                        </div>

                        <div class="col-md-4">
                            <label class="form-label fw-bold">Tipo de Combo <span class="text-danger">*</span></label>
                            <select name="tipo" id="tipo" class="form-select" required>
                                <option value="fijo" {{ old('tipo', $combo->tipo) === 'fijo' ? 'selected' : '' }}>Precio Fijo Especial</option>
                                <option value="condicionado" {{ old('tipo', $combo->tipo) === 'condicionado' ? 'selected' : '' }}>Gratuito Condicionado (Suma normal)</option>
                            </select>
                            <div class="form-text">
                        ...
                        <div class="col-md-4" id="precio-container">
                            ...
                            <div class="input-group">
                                ...
                                <input type="number" name="precio_total" id="precio_total" class="form-control" step="0.01" min="0" placeholder="Ej: 45.00" value="{{ old('precio_total', $combo->precio_total) }}">
                            </div>
                        </div>

                        <div class="col-md-4 d-flex align-items-center pt-4">
                            <div class="form-check form-switch">
                                <input class="form-check-input" type="checkbox" name="activo" id="activo" value="1" {{ old('activo', $combo->activo) ? 'checked' : '' }}>
                    ...
                    <div class="d-flex justify-content-end gap-2 border-top pt-4">
                        ...
                        <button type="submit" class="btn btn-primary px-5"><i class="bi bi-save"></i> Guardar Cambios</button>
                    </div>
                </form>
            </div>
        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    @if(old('items'))
        ...
        @endforeach
    @elseif($combo->items->isNotEmpty())
        @foreach($combo->items as $item)
            addProductRow(
                "{{ $item->producto_id }}",
                "{{ $item->cantidad }}",
                {{ $item->es_gratuito ? 'true' : 'false' }}
            );
        @endforeach
    @else
        addProductRow();
